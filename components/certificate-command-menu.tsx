...
interface CertificateCommandMenuProps {
  onSearch?: (query: string) => void;
}

export function CertificateCommandMenu({ onSearch }: CertificateCommandMenuProps) {
    const [open, setOpen] = React.useState(false)
    const [searchResults, setSearchResults] = React.useState<Certificate[]>([])
    ...
    const handleCertificateSelect = (certificate: Certificate) => {
      router.push(`/${locale}/about/certificates/${certificate.slug}`)
      setOpen(false)
    }
    ...
    return (
      <div>
        ...
        >
          ...
          <span className="text-gray-500">{t('searchCertificates')} </span>
          ...
          <CommandList>
            ...
            {searchResults.length > 0 && (
              <CommandGroup heading={t('certificates')}>
                {searchResults.map((certificate) => (
                  <CommandItem 
                    key={certificate.id}
                    onSelect={() => handleCertificateSelect(certificate)}
            ...
            <CommandGroup heading={t('quickActions')}>
              ...
              <CommandItem onSelect={() => handleQuickSearch('featured')}>
                {t('featuredCertificates')}
              </CommandItem>
              <CommandItem onSelect={() => router.push(`/${locale}/about/certificates`)}>
