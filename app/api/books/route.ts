...
export async function GET(request: NextRequest) {
  try {
    ...
    const laravelUrl = new URL(`${LARAVEL_API_URL}/books`);
    ...
    if (min_price) laravelUrl.searchParams.set('min_price', min_price);
    if (max_price) laravelUrl.searchParams.set('max_price', max_price);
