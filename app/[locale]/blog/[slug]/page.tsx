...
export const revalidate = 60 // ISR: her 60 saniyede bir yeniden generate edilir
...
export default async function BlogDetailPage({ params }: BlogPageProps) {
  ...
  if (!blog) {
    return (
      <section className="py-32 container mx-auto px-4">
        <h1 className="text-2xl font-bold">Blog bulunamadı</h1>
        <Link
          href={`/${locale}/blog`}
          className="mt-4 inline-block px-6 py-3 bg-green-700 text-white rounded-lg hover:bg-green-800 transition-colors"
        >
          ← Tüm Blog Yazıları
        </Link>
      </section>
  ...
  const metaDescription =
    blog.content?.replace(/<[^>]*>/g, "").substring(0, 160) || "Blog yazısı"
  const canonicalUrl = `https://elazadarya.com/${locale}/blog/${blog.slug}`
  ...
  const blogSchema = {
    ...
    image: blog.cover_image
      ? blog.cover_image.startsWith("http")
        ? blog.cover_image
        : `https://bioenerjist-books.s3.amazonaws.com/${blog.cover_image}`
      : "/ogm.png",
    datePublished: blog.published_at,
    dateModified: blog.updated_at,
    author: { "@type": "Person", name: "Elza Darya" },
