      ...
      <section className="py-32 container mx-auto px-4 mb-">
        ...
        <div className="mb-8 flex flex-col sm:flex-row gap-4 sm:justify-between sm:items-center">
          <div className="flex-1 max-w-md ">
            <CertificateCommandMenu />
          </div>
          <div className="flex-shrink-0">
            <CertificateFilterDrawer />
          </div>
        </div>
        ...
        {certificates.length > 0 ? (
          <div className="grid gap-x-4 gap-y-8 md:grid-cols-2 lg:gap-x-6 lg:gap-y-12 2xl:grid-cols-3">
            {certificates.map((certificate) => {
              ...
              const certificateSlug = certificate.slug || certificate.name
                .toLowerCase()
                .replace(/[^\w\s-]/g, '')
                .replace(/\s+/g, '-')
                .trim()
              ...
              <Link key={certificate.id} href={`/${locale}/about/certificates-and-diploma/${certificateSlug}`} className="group flex flex-col">
                <div className="mb-4 flex overflow-clip rounded-xl md:mb-5">
                  ...
                  {certificate.file_url ? (
                      ...
                      <img src="/images/block/placeholder-dark-1.svg" alt={certificate.name} className="aspect-3/2 h-full w-full object-cover object-center" />
                    )}
                    <div className={`fallback-image aspect-3/2 h-full w-full bg-gray-200 flex items-center justify-center ${certificate.file_url ? 'hidden' : 'flex'}`}>
                ...
                <div className="text-muted-foreground mb-4 flex items-center gap-2 text-xs">
                  ...
                  <span>•</span>
                  <span>{new Date(certificate.created_at).toLocaleDateString('tr-TR', { 
                    day: 'numeric', 
                    month: 'short', 
                    year: 'numeric' 
                  })}</span>
          ...
          <div className="text-center py-12">
            <p className="text-muted-foreground text-lg">{t('noCertificates')}</p>
          </div>
        )}
